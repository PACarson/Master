...
## v1.9 — Stable, 2026-08-07

**Status:** Ratified. MINOR bump from v1.8. One addition: **§0.6 AI Engineering Protocol, item 3, gains explicit per-file export discipline** — implement → **export/persist immediately** → self-check → surface → fix → resubmit, instead of implement → self-check → ... with export left implicit/deferred. Motivated by direct incident evidence, not a speculative pattern: an AI coding session wrote multiple Engine/`.js` files in one sitting, deferred export to the end, and the session ended before that batched export happened — every file finished since the last export was lost, not just the one in progress. This is a checklist addition to an existing Tier 1 protocol section (§0.6 is already `VALIDATED`), not a lifecycle, Engineering Principle, or document-tier change — no ecosystem-level ADR required per §0.8's own MINOR criteria ("new checklist item... no ADR required, just a note in the ecosystem changelog"). Not staged as a Candidate Pattern either: that mechanism (§4, D7) exists for architecture claims awaiting second-project convergence — this is a workflow safety rule with immediate, first-hand evidence of the failure it prevents, so it's ratified directly rather than held pending.
**Property OS status:** unchanged from v1.8 — significant progress since v1.7: 910_PropertyAssetEngine and the full 990-996 test suite (141 tests) are now confirmed passing against a real GAS project, not just self-checked (two real issues found and fixed along the way: a stale-file deployment sync gap, and a genuine execution-timeout fix in `ensureSheetSchema_`'s per-execution caching — both documented in Property OS's own `00_Project_State.js`). 914_FinanceEngine's Vertical Slice is now fully APPROVED (ADR-P13 — Event Completeness Principle applied, `TransactionType` resolved to 4 values including a dedicated `Reversal`), Runtime not yet written. Obligation Engine sub-system remains `pending` per §0.5 — real concurrency, Cache TTL expiration, and schema-drift detection are still genuinely unverified, not glossed over just because everything else passed.
