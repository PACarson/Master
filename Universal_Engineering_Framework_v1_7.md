...
## v1.7 — Stable, 2026-07-29

**Status:** Ratified. MINOR bump from v1.6, same day. One addition: **§2 Architecture's Platform Constraints gains a stated future direction for EventBus** — Property OS's project owner, resolving how 914_FinanceEngine should relate to ADR-P07's still-placeholder EventBus, stated that the EventBus itself should eventually be one independent, shared Platform Capability across every Domain OS, not something each project reimplements. Recorded as stated intent, explicitly not claimed as a ratified UCR (it hasn't cleared that bar — this is one ecosystem owner's direction, not two independent projects converging) — but recorded now because it shapes how every Domain OS should shape its own Adapter today, before the shared EventBus exists.
**Property OS status:** unchanged — Obligation Engine sub-system `pending` per §0.5; 910_PropertyAssetEngine Runtime complete, tests self-checked (139/139), not yet confirmed against a real GAS run; 914_FinanceEngine entering Contract Design under ADR-P12 (Property OS's own log) — built against EventBus's event *shape*, with `publishPropertyEvent_()`/`subscribeFinanceEvent_()` both permitted as placeholder Adapters until the real Shared EventBus exists.
