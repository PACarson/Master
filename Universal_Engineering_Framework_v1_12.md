...
## v1.12 — Stable, 2026-08-16

**Status:** Ratified. MINOR bump from v1.11. §0.6 AI Engineering Protocol, item 3 (per-file export/persist, added v1.9) is extended rather than replaced: a new sub-step requires confirming the persisted copy is actually readable before moving on — v1.9 mandated the export *action* itself but not a read-back check on it. A new item 4 generalizes the same discipline upward: **File Checkpoint → Engine Checkpoint → Sprint Checkpoint**, each additive, none replacing the one below it, with the "implement everything, export once at the end" workflow now named and explicitly prohibited at every level rather than left to be inferred from item 3's file-scoped wording. Motivated by a second, separate incident of the same failure shape v1.9 already fixed at the file level: multi-file, Sprint-scoped work was lost to a container reset because no checkpoint existed above the file level, and nothing recorded which files had actually cleared item 3's export step versus which hadn't — item 3 being followed correctly file-by-file doesn't, by itself, answer "what has this Sprint actually completed and verified so far," which is the gap item 4 and the new ecosystem-level `Universal-Recovery-Manifest.md` close. Classified MINOR on the same reasoning v1.9 itself used: a checklist addition to an already-`VALIDATED`-tagged Tier 1 protocol section (§0.6), not a lifecycle, Engineering Principle, or document-tier change. The Recovery Manifest is deliberately introduced as a required *artifact of §0.6's process*, not a new §0.3 Conditional Document — the latter reading would arguably force a MAJOR bump and an ecosystem-level ADR over a document-tier change; the former stays inside §0.8's explicit "new template... no ADR required" MINOR criteria, which is the classification used here. Flagged rather than silently decided, consistent with D8's own precedent for a genuinely arguable classification call — revisit if a future review disagrees with treating this as MINOR.
**Property OS status:** unchanged from v1.10 — significant progress since v1.7: 910_PropertyAssetEngine and the full 990-996 test suite (141 tests) are now confirmed passing against a real GAS project, not just self-checked (two real issues found and fixed along the way: a stale-file deployment sync gap, and a genuine execution-timeout fix in `ensureSheetSchema_`'s per-execution caching — both documented in Property OS's own `00_Project_State.js`). 914_FinanceEngine's Vertical Slice is now fully APPROVED (ADR-P13 — Event Completeness Principle applied, `TransactionType` resolved to 4 values including a dedicated `Reversal`), Runtime not yet written. Obligation Engine sub-system remains `pending` per §0.5 — real concurrency, Cache TTL expiration, and schema-drift detection are still genuinely unverified, not glossed over just because everything else passed.
**Property OS status:** significant progress since v1.7 — 910_PropertyAssetEngine and the full 990-996 test suite (141 tests) are now confirmed passing against a real GAS project, not just self-checked (two real issues found and fixed along the way: a stale-file deployment sync gap, and a genuine execution-timeout fix in `ensureSheetSchema_`'s per-execution caching — both documented in Property OS's own `00_Project_State.js`). 914_FinanceEngine's Vertical Slice is now fully APPROVED (ADR-P13 — Event Completeness Principle applied, `TransactionType` resolved to 4 values including a dedicated `Reversal`), Runtime not yet written. Obligation Engine sub-system remains `pending` per §0.5 — real concurrency, Cache TTL expiration, and schema-drift detection are still genuinely unverified, not glossed over just because everything else passed.
...
| §0.6 AI Engineering Protocol | **VALIDATED** (items 1-3, since v1.9) / DRAFT — unexercised (item 4, new in v1.12) | Items 1-3 followed every session this cycle. Item 4's Engine/Sprint checkpoint hierarchy and the Recovery Manifest are freshly drafted from this version's motivating incident — no Engine- or Sprint-level checkpoint has actually been exercised under it yet |
...
| `00_Project_State.js` | Living changelog: version, in-progress, bugs, next steps, file/Engine/Sprint checkpoint status (§0.6 item 4, new in v1.12) | ✅ all 3 projects *(as `.txt`)* |
...
3. **Process (per file)** — implement → **export/persist immediately, before anything else** → **verify the persisted copy exists and is readable** → self-check → surface what was found → fix → resubmit. Never let a completed or modified file sit unexported while work continues on the next one. *(Added v1.9 — direct incident evidence: a session wrote multiple Engine/`.js` files, deferred export to session end, and the session ended first; every file finished since the last export was lost, not just the one in progress. Export is now the second action after implement, ahead of even self-check, so completed work is safe before any further step can go wrong.)* *(Verify sub-step added v1.12 — a write call reporting success is not itself evidence the file is recoverable; confirm by reading the persisted copy back, not by trusting the write call's return value alone.)*
4. **Checkpoint hierarchy** — item 3's per-file discipline is the base unit, not the whole rule. **File Checkpoint → Engine Checkpoint → Sprint Checkpoint**: once every file belonging to an Engine (a cohesive multi-file unit — `910_PropertyAssetEngine` and its own tests, for example) is individually export/persist/verified per item 3, record an Engine Checkpoint; once every Engine belonging to a Sprint is checkpointed, record a Sprint Checkpoint. A later checkpoint is additive — it never replaces or substitutes for the file-level checkpoints beneath it. The working container, AI session, conversation context, or any temporary filesystem is never the authoritative store, at any level of this hierarchy — assume the container can reset, the session can end, context can be lost, or the AI agent can become unavailable, mid-Engine or mid-Sprint as readily as mid-file. Consequently: **implement several files (or several Engines), then export/persist only once at the end, is prohibited** — at every level, even when nothing goes wrong. Item 3 already excludes this at the file level; this item states it explicitly at the Engine and Sprint levels too, rather than leaving it to be inferred. Checkpoint level and status get recorded in that project's `00_Project_State.js` (§0.2) for single-project work, and additionally in the ecosystem-level `Universal-Recovery-Manifest.md` (see `00-INDEX.md`) for any work spanning more than one project/OS or more than one AI session. *(Added v1.12 — direct incident evidence: a second, Sprint-scoped incident of the same failure shape item 3 already fixed at the file level — a container reset lost multi-file Sprint work because no checkpoint existed above the file level, and no record existed of which files had actually cleared item 3 versus which hadn't.)*
5. **After implementing** — sync governance docs the same session, never a deferred pass.
...
**Historical version retention** — *(revised v1.11, see Status line above for what changed and why)* a historical governance version's status is provenance/audit trail, not a runtime dependency of the current baseline. The current baseline (latest version) must be self-contained: every Decision/Rule appears there in full — Context/Options/Decision/Evidence complete — not as a "see vX.X" stub; an inline `Origin: vX.X` tag is a birth-record, not a pointer the reader must follow to understand the rule. A historical version being tagged by `Origin` does not by itself require permanent retention in place — it means that file is where fidelity to the original can be checked if anyone ever needs to. Before treating a historical version as pure, lower-priority provenance (eligible to move toward `Historical/Unreferenced/`), verify — don't assume — that the current baseline's version of that Decision is actually faithful to the original: diff it. This check is only required when a version bump actually modifies, compresses, rewrites, or migrates an existing Decision's Context/Options/Decision/Evidence — not for typos, formatting, meaning-preserving wording tweaks, or unrelated new sections; otherwise every routine edit generates Fidelity busywork and turns UEF into governance maintained for its own sake. Record the result in that Decision's own Evidence, or a `Fidelity` field. Archive ≠ delete still holds, but the reason shifts: a version isn't kept because the current baseline depends on it to be understood, it's kept because the audit trail itself has standing value — a lower bar that allows earlier archiving, but deletion beyond archiving still needs a separate, explicit human call this rule doesn't make automatically.
...
**EventBus as a future independent Platform Capability** *(new in v1.7, stated direction — not yet built, not a ratified UCR)* — UCR7 already establishes that each project isolates its infrastructure dependencies behind a single Adapter. Personal AI Core's stated direction (Property OS's project owner, 2026-07-29, while resolving how 914_FinanceEngine should relate to ADR-P07's still-placeholder EventBus) takes this one step further at the ecosystem level: **the EventBus itself should eventually be one independent, shared Platform Capability used by every Domain OS, not something each project reimplements.** Every Domain OS talks to it only through its own publish/subscribe Adapter pair (e.g., `publishXEvent_()`/`subscribeXEvent_()`); a future transport swap (Sheets → Firestore → Pub/Sub → anything else) then touches only Adapters, never any Domain's Runtime. This is explicitly a stated intent for where Personal AI Core's architecture is headed, not a pattern two independent projects have already converged on — it doesn't clear the same evidence bar as a UCR, and isn't claimed to. It's recorded here because it directly shapes how every current and future Domain OS should structure its own Adapter now, even before the shared EventBus exists: **build against the event's shape, not against how it currently arrives**, so nothing needs rewriting later, only re-pointing.

**Event Completeness Principle** *(new in v1.8, stated direction — not yet built ecosystem-wide, not a ratified UCR)* — a companion to the EventBus note above, from the same resolution session (Property OS's project owner, 2026-07-29, deciding how 914_FinanceEngine should get an Obligation payment's Category without calling back into 912). Stated principle: **a Domain Event should carry the stable business data its known consumers need to complete their own work, rather than requiring them to call back into the publisher's Truth Layer or read API.** A read-only query still couples a consumer to the publisher's *process*, not just its data — the moment two Domain OS projects are genuinely separate deployments (already true elsewhere in this ecosystem), a direct function call across that boundary isn't just discouraged, it's impossible, while a complete event payload works identically either way. Same evidentiary status as the EventBus note: this is CC's direction for how every current and future Domain OS should shape its own Event Contracts, not yet something two independent projects have converged on independently — recorded so it shapes design now rather than being rediscovered project by project. Property OS's own applied instance: ADR-P13 (`category` added to `PAYMENT_COMPLETED`/`PAYMENT_REVERSED`).
...
**Glossary** — *Truth Layer*, *Single Calculation Source*, *Domain Ownership*, *Documentation Drift*, *Append-only Contract*, *Engine* (a cohesive multi-file functional unit, typically a numbered-prefix module plus its own test file(s) — `910_PropertyAssetEngine`, `914_FinanceEngine`), *Sprint* (a planned batch of Engine-building work treated as one persistence-checkpoint scope under §0.6 item 4 — new term, formalized in v1.12, not previously defined in this document), *Checkpoint* (§0.6 item 4's persistence/recovery sense — a file, Engine, or Sprint confirmed persisted and verified; unrelated to the Failure Catalog's `_syncDailyShifts_` entry below, which uses the same word for a status-branch validation gate in running code — the two meanings aren't connected) — defined once at ecosystem level, referenced from each project.
...
### D10 — EventBus gap: Platform Infrastructure limitation, not a reason to change Domain Architecture *(added in v1.7)*
**Question:** 914_FinanceEngine's design depends on ADR-P01's "subscribes to Domain Events" requirement, but the real EventBus (ADR-P07) is still a Logger placeholder with no actual dispatch mechanism. Does Finance Engine's Architecture change to accommodate this gap (e.g., a temporary direct call from the publishing Engine into Finance, bypassing the event-subscription model), or does the Architecture stay as designed while the gap is treated purely as an Infrastructure limitation?
**Options:** (a) Change Finance Engine's Architecture — have it read another Engine's Truth Layer directly, or accept a hard-coded direct call from 912/910, until EventBus is real · (b) Keep Finance Engine's Architecture exactly as ADR-P01 specifies (subscribe to events, no direct Truth access, no own schedule), and treat the missing dispatch mechanism purely as an unfinished Infrastructure Adapter (`subscribeFinanceEvent_()`, mirroring `publishPropertyEvent_()`), built against the event's *shape* regardless of how it currently arrives
**Decision:** (b) — Property OS's project owner, explicitly framing this as an Architecture Decision rather than a Runtime one. Stated principle, kept verbatim as the clearest summary: "Platform 未完成，不应改变 Domain。Infrastructure 可以 Placeholder。Architecture 不允许 Placeholder。" An incomplete Platform must not change Domain design; Infrastructure may be a placeholder; Architecture may not be. This is UCR7's Adapter-isolation pattern applied to the *consuming* side of an event (not just the publishing side `publishPropertyEvent_()` already covered) — a new, symmetric Adapter (`subscribeFinanceEvent_()`) absorbs the gap instead of the Domain Layer compromising to work around it.
**Impact:** §2 Architecture's Platform Constraints gains the EventBus-as-future-Platform-Capability note (this same session). Property OS's own `00_ADR_Log.js` ADR-P12 records the applied instance in full. No change to UCR7 itself — this is UCR7's existing scope, extended to a new call site, not a new pattern requiring fresh evidence.
**Review Trigger:** Once a real Shared EventBus API exists anywhere in the ecosystem, revisit whether `subscribeFinanceEvent_()`-style Adapters across projects converge on a common shape worth promoting further (UCR7 already covers the isolation principle; a common subscription-Adapter *shape* would be a separate, later question).

### D11 — Event Completeness Principle; Reversal gets its own TransactionType *(added in v1.8)*
**Question:** Two related open items from 914_FinanceEngine's Vertical Slice. (1) Finance Engine needs an Obligation payment's Category to build a Ledger entry — read-only callback into 912, or does the event carry it? (2) Is a sale-reversal's Ledger effect an `Expense` offsetting the original `Income`, or its own thing?
**Options for (1):** (a) `getObligation()` read-only lookup — technically doesn't violate "never write another Engine's Truth Layer" · (b) event payload carries `category` directly, no cross-Engine call of any kind
**Options for (2):** (a) `Expense`-tagged offsetting entry (simpler, only 2 TransactionType values) · (b) a dedicated `Reversal` TransactionType, with cashflow queries netting it against whatever it reverses
**Decision:** (1b) and (2b) — Property OS's project owner. For (1): stated as a named principle, **Event Completeness** — a Domain Event should carry the stable data its known consumers need, not require a callback into the publisher. A read-only query still couples the consumer to the publisher's *process*; the moment Domain OS projects are genuinely separate deployments (already true elsewhere in this ecosystem), a direct call isn't just discouraged, it's impossible — an event payload works identically either way. For (2): a reversal is not a cost and not revenue, it's an undone prior fact; collapsing it into `Expense` would corrupt future capital-gains/tax analysis that needs to see "it happened" and "it was undone" as distinct.
**Impact:** §2 gains the Event Completeness Principle note, same tier as D10's EventBus note (stated direction, not yet two-project-evidenced). Property OS's applied instance: ADR-P13 — `category` added to `PAYMENT_COMPLETED`/`PAYMENT_REVERSED`'s required fields (903, with 912's `recordPayment`/`reversePayment` updated to supply it from data already in scope), `TransactionType` resolved to a 4-value enum (`Income`/`Expense`/`Adjustment`/`Reversal`) in 914's schema.
**Review Trigger:** If a second project's Event Contract independently converges on "carry what consumers need, don't make them call back," that's the evidence needed to consider promoting this past stated-direction status.
