...
## v1.11 — Stable, 2026-08-07

**Status:** Ratified. MINOR bump from v1.10, same day. §0.8's historical-version-retention rule (added last version) is revised, not just extended: v1.10's framing — "must be retained while referenced" — conflated an internal provenance tag with an active file dependency. Verified directly against the case that prompted the original rule: `D8`'s full text is byte-identical between `Universal_Engineering_Framework_v1_5.md` and this document, confirming the current baseline was already self-contained for that Decision — the historical file was never actually load-bearing for interpreting it. Revised rule: current baseline must be self-contained (already true in practice); historical versions are provenance/audit trail, not a runtime dependency; before demoting a version toward `Historical/Unreferenced/`, diff it against the current baseline's version of any Decision it originated — but only when that Decision's substance was actually modified in a later version, not for every edit. Still MINOR, no ADR — a revision within an existing §0.8 rule, not a lifecycle/principle/document-tier change.
...
3. **Process (per file)** — implement → **export/persist immediately, before anything else** → self-check → surface what was found → fix → resubmit. Never let a completed or modified file sit unexported while work continues on the next one. *(Added v1.9 — direct incident evidence: a session wrote multiple Engine/`.js` files, deferred export to session end, and the session ended first; every file finished since the last export was lost, not just the one in progress. Export is now the second action after implement, ahead of even self-check, so completed work is safe before any further step can go wrong.)*
