...
## v1.10 — Stable, 2026-08-07

**Status:** Ratified. MINOR bump from v1.9, same day. One addition: **§0.8 Version Governance gains an explicit historical-version-retention rule.** Any historical governance version still referenced by the current baseline — directly (an inline `(decided/added/new in vX.X)` tag) or indirectly (a chain of such tags, e.g. v1.9→v1.7 D10→v1.6 D9→v1.5 D8) — must be retained; archiving into a `Historical/` location is fine once a flat folder gets unwieldy, deletion of an actively-referenced version is not. This session's own v1.5→v1.9 chain is the concrete case that surfaced the gap — nothing in UEF previously said versions couldn't just be deleted once superseded, and this document's own D8/D9/D10 tags would become unverifiable assertions (EP5 violation) if their cited source versions were gone. No ADR required per this same §0.8 — a new rule within an existing section, not a lifecycle/principle/document-tier change.
**Property OS status:** unchanged from v1.9 — significant progress since v1.7: 910_PropertyAssetEngine and the full 990-996 test suite (141 tests) are now confirmed passing against a real GAS project, not just self-checked (two real issues found and fixed along the way: a stale-file deployment sync gap, and a genuine execution-timeout fix in `ensureSheetSchema_`'s per-execution caching — both documented in Property OS's own `00_Project_State.js`). 914_FinanceEngine's Vertical Slice is now fully APPROVED (ADR-P13 — Event Completeness Principle applied, `TransactionType` resolved to 4 values including a dedicated `Reversal`), Runtime not yet written. Obligation Engine sub-system remains `pending` per §0.5 — real concurrency, Cache TTL expiration, and schema-drift detection are still genuinely unverified, not glossed over just because everything else passed.
...
**Historical version retention** — any historical governance version that remains referenced by the current governance baseline, directly or indirectly, must be retained. Historical versions may be archived, but must not be deleted while they remain part of the traceable decision lineage. Archive ≠ delete: once a flat folder gets unwieldy, historical versions move to a `Historical/` location; a version with zero remaining references from the current baseline may move further, to `Historical/Unreferenced/` — but nothing gets deleted outright while any current document still cites it as a source. *(Added v1.10 — this document's own inline `(decided/new/added in vX.X)` tags and D8/D9/D10 entries are exactly this kind of reference: each points to a specific historical version as its traceable source, so deleting that version would turn the tag into an unverifiable assertion, violating EP5.)*
