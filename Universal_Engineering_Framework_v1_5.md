...
## v1.5 — Stable, 2026-07-29

**Status:** Ratified. MINOR bump from v1.4, same day. Two changes:

1. **File extension default: `.txt` → `.js` (D8).** Property OS's project owner made this call directly, resolving the open conflict v1.4 flagged a few hours earlier rather than leaving it for a future pass. Every project-level governance/Runtime file this framework produces now defaults to `.js`. This does *not* retroactively rename Rider OS's, Reminder OS's, or Personal AI Core's existing `.txt` files — those migrate on their own schedule, tracked as an Evolution item, not silently implied by this document. Property OS itself is fully on `.js` as of this version (it started on `.gs` before this decision, its owner's original instruction predating this document's involvement — see D8 for how that's reconciled, not glossed over).
2. Property OS is the first genuinely new project — not a retrofit — to reach real Runtime code under this framework since Rider OS's pilot, and (separately from the extension question) contributes two kinds of engineering-pattern evidence that get treated differently on purpose:
   - **Infrastructure Adapter / Port isolation** — independently arrived at (ADR-P07, isolating EventBus behind `publishPropertyEvent_()`) without reference to Rider OS's `00_Business_Rules.js`. Genuine convergent evidence from **two independent projects** on the same pattern (Rider OS's UCR6/TruthEngine is the other), clearing the bar for a MINOR promotion — generalized as **UCR7**.
   - **Everything else Property-OS-specific** (Event Immutability + Compensating Events chief among them) is one project's evidence only and does *not* clear that bar. Per **D7**, recorded in a new **Candidate Patterns** section rather than folded in or dropped.

One Knowledge Base Failure Catalog entry (Sheets silently coercing a written ISO date string to a Date serial on read) is added on its own, lighter precedent — the Catalog has always accepted single-instance evidence (BUG-2, BUG-11, etc. are each sourced from one bug), unlike UCR/EP-level changes.

**Property OS status:** Architecture + Contract Design phase complete and formally approved (Constitution, ADR Log, Domain Model, Obligation Engine Vertical Slice — ADR-P01 through P07, P03 reserved not locked). Foundation layer + Obligation Engine/Scheduler Runtime written and confirmed running against a live GAS project (schema creation and a write path both succeeded on direct test) — no formal Test Plan or Review has run yet, so this is "runs" not "verified correct," and it's too early for the kind of full retrospective retrofit Rider OS received under D5. All ten `.gs` files renamed to `.js` this same pass, per D8.
