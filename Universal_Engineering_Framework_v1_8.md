...
## v1.8 — Stable, 2026-07-29

**Status:** Ratified. MINOR bump from v1.7, same day. One addition: **§2 Architecture's Platform Constraints gains a stated Event Completeness Principle** — a companion to v1.7's EventBus note, from the same resolution session: Domain Events should carry the stable data their known consumers need, rather than requiring a callback into the publisher. Same evidentiary tier as the EventBus note — CC's stated direction for this ecosystem's Event Contracts, not yet two-project-evidenced.
